...
contract MinterV2 is Auth {
    ...
    address internal burnedAirdrop;
    ...
    constructor(
        ...
        burnedAirdrop = burnedAirdrop_;
        active_period = (block.timestamp + (1 * week)) / week * week;
    }

    function initialize(
        address newAirdrop_ // sum amounts / max = % ownership of top protocols, so if initial 20m is distributed, and target is 25% protocol ownership, then max - 4 x 20m = 80m
    ) external {
        //setup initial mint params here, lock team as ve nft's
        //setup fund team vesting locks
        require(initializer == msg.sender);
        _token.mint(newAirdrop_, 3_000_000e18);
    ...
    function circulating_supply() public view returns (uint) {
        return _token.totalSupply() - _ve.totalSupply() - _token.balanceOf(burnedAirdrop) - _token.balanceOf(airdrop) - _token.balanceOf(owner);
    }
