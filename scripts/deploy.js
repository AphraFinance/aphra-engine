async function main() {
  ...
  const Authority = await ethers.getContractFactory("MultiRolesAuthority");
  const AirdropClaim = await ethers.getContractFactory("AirdropClaim");

  const Token = await ethers.getContractFactory("AphraToken");
  const VaultFactory = await ethers.getContractFactory("VaultFactory");
  const VaderGateWay = await ethers.getContractFactory("VaderGateway");
  const Gauges = await ethers.getContractFactory("GaugeFactory");
  const Bribes = await ethers.getContractFactory("BribeFactory");
  const Ve = await ethers.getContractFactory("contracts/veAphra.sol:veAphra");
  const Ve_dist = await ethers.getContractFactory(
    "contracts/ve_dist.sol:ve_dist"
  );
  const BaseV1Voter = await ethers.getContractFactory("Voter");
  const BaseV1Minter = await ethers.getContractFactory("Minter");

  const authority = await Authority.deploy(GOVERNANCE, "0x");
  const token = await Token.deploy(GOVERNANCE, authority);
  const gauges = await Gauges.deploy();
  const bribes = await Bribes.deploy();
  const ve = await Ve.deploy(token.address, GOVERNANCE, authority);
  const ve_dist = await Ve_dist.deploy(ve.address);
  const vaderGateway = await VaderGateWay.deploy(
    VADER_MINTER,
    GOVERNANCE,
    authority,
    VADER_ADDR,
  ...
  const airdropClaim = await AirdropClaim.deploy(
    "0xd0aa6a4e5b4e13462921d7518eebdb7b297a7877d6cfe078b0c318827392fb55",
    ve.address
  );

  //setup roles and then test deploy
